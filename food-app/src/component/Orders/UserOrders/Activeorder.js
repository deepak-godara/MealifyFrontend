...
const Activeorder = ({ item, socket }) => {
  ...
  if (socket) {
    socket.on('deliveryConfirmationRequestOwner', ({ UserId, OwnerId, OrderId }) => {
      if(OrderId === item._id) {
        dispatch(saveOrderStatus({ orderId: OrderId, status: "deliveryConfirmByUser" }));
  ...
  const onConfirm = () => {
    ...
    if (socket) {
      socket.emit("deliveryConfirmationByUser", { orderId: item._id, HotelId: item.HotelId, userId: item.UserId, status: "delivered" });
    } else {
  ...
  useEffect(() => {
    // console.log("changed order form saveOrderStatus is:", order);
    console.log("status Updated");
    console.log(preparing);
    console.log(outForDelivery);
  }, [dispatch, order, delivered , preparing , outForDelivery , statusDisplay]);
