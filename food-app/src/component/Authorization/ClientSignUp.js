...
const ClientSignReducer=(state=InitialState,action)=>{
    ...
    else if(action.type==='initalUserName')
    {
        ...
        else if(NewState.UserName==='UserName')
        NewState.UserName='';
    }
    else if(action.type==='initalPassword')
    {
        ...
        else if(NewState.Password==='Password')
        NewState.Password='';
    }
...
function ClientSignUp(props) {
    const [ClientSignData,SetClientSignData]=useReducer(ClientSignReducer,InitialState);
    ...
    const ClientChange=(event)=>{
        ...
        if(ErrorData.err==='true')
        SetErrorData({type:'noerror'});
    }
    const ClientBlurFocus=(event)=>{
         ...
         SetClientSignData({type:'initalEmail'});
         if((ErrorData.err==='true'))
         SetErrorData({type:'noerror'});
    }
...
props.onChangeLogin();
  ...
  return (
    <form className='add-client1-form' onSubmit={ClientDataSubmit}>
        {ErrorData.err==='true'&&<div className='login-error-options'>{ErrorData.message}</div>}
        
        <div className='add-client1-div'>
            <label className='add-client1-label'>UserName:</label>
            <input className='add-client1-input' type='text' value={ ClientSignData.UserName} name="addUserName" onChange={ClientChange} onFocus={ClientBlurFocus} onBlur={ClientBlurFocus}></input>
        </div>
        <div className='add-client1-div'>
            <label className='add-client1-label'>Email:</label>
            <input className='add-client1-input' type='email' value={ ClientSignData.Email} name="addEmail" onChange={ClientChange} onFocus={ClientBlurFocus} onBlur={ClientBlurFocus}></input>
        </div>
        <div className='add-client1-div'>
            <label className='add-client1-label'>Password:</label>
            <input className='add-client1-input' type='password' value={ ClientSignData.Password} name="addPassword" onChange={ClientChange} onFocus={ClientBlurFocus} onBlur={ClientBlurFocus}></input>
        </div>
        <div className='add-client1-div'>
            <label className='add-client1-label'>ConfirmPassword:</label>
            <input className='add-client1-input' type='password' value={ ClientSignData.ConfirmPassword} name="addConfirmPassword" onChange={ClientChange} onFocus={ClientBlurFocus} onBlur={ClientBlurFocus}></input>
        </div>
